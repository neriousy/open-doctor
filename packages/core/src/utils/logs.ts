...
export interface LogService {
  readonly discover: (dbPath?: string) => LogSource[]
  readonly read: (source: LogSource, options?: { maxLines?: number }) => LogEntry[]
  readonly summarize: (sources?: LogSource[]) => LogSummary
}

export class Logs extends Context.Service<Logs, LogService>()("@open-doctor/core/Logs") {}

export const LogsLayer = Layer.effect(
  Logs,
  Effect.sync(() =>
    Logs.of({
      discover: discoverLogSources,
      read: readLogEntries,
      summarize: summarizeLogs,
    }),
  ),
)

export const defaultLayer = LogsLayer
