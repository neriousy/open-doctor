...
import { Database, tableColumns } from "../db/sqlite.js"
...
export const listArchivedSessions = Effect.fn("Utils.sessions.archived")(function* (filename: string) {
  ...
  return yield* database.withDatabase(filename, (db) =>
    Effect.gen(function* () {
      yield* database.requireTable(db, "session")
      yield* database.requireColumns(db, "session", ["id", "title", "directory", "time_updated", "time_archived"])
...
export const unarchiveSession = Effect.fn("Utils.sessions.unarchive")(function* (
  ...
  return yield* database.withDatabase(input.db, (db) =>
    Effect.gen(function* () {
      yield* database.requireTable(db, "session")
      yield* database.requireColumns(db, "session", ["id", "time_archived"])
