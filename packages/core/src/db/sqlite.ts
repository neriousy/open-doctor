...
export interface DatabaseService {
  readonly withDatabase: <A, E>(
    filename: string,
    use: (db: SqliteDatabase) => Effect.Effect<A, E | ToolkitError, never>,
  ) => Effect.Effect<A, E | ToolkitError, never>
  readonly inspect: (db: SqliteDatabase) => Effect.Effect<DatabaseState>
  readonly backup: (db: SqliteDatabase, filename: string) => Effect.Effect<string>
  readonly transaction: (db: SqliteDatabase, changes: SqlChange[]) => Effect.Effect<void>
  readonly requireTable: (db: SqliteDatabase, table: string) => Effect.Effect<void, ToolkitError>
  readonly requireColumns: (db: SqliteDatabase, table: string, columns: string[]) => Effect.Effect<void, ToolkitError>
}

export class Database extends Context.Service<Database, DatabaseService>()("@open-doctor/core/Database") {}
...
export const defaultLayer = DatabaseLayer
