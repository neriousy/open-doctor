...
import { Database } from "@open-doctor/core/db/sqlite"
import type { SqlChange } from "@open-doctor/core/db/sqlite"
import { formatError } from "@open-doctor/core/error"
import { defaultDataDir, resolveDbArg } from "@open-doctor/core/input"
import { plannedWorkspaceRepairs } from "@open-doctor/core/repairs/no-such-column-name"
import { Backup } from "@open-doctor/core/utils/backups"
...
export async function scanToolkitHealth(dbPath = resolveDbArg()): Promise<ToolkitHealth> {
  ...
  const backup = runCoreSync(Effect.gen(function* () {
    return yield* Backup
  }))
  const logs = runCoreSync(Effect.gen(function* () {
    return yield* Logs
  }))
  const backupCount = backup.list(dbPath).length
  const logSources = logs.discover(dbPath)
  const logSummary = logs.summarize(logSources)
...
async function workspaceRepairHealth(dbPath: string): Promise<WorkspaceRepairHealth> {
  try {
    const changes = await runCorePromise(
      Effect.gen(function* () {
        const database = yield* Database
        return yield* database.withDatabase(dbPath, (db) =>
          Effect.gen(function* () {
            const state = yield* database.inspect(db)
...
async function archivedSessionCount(dbPath: string) {
  try {
    const sessions = await runCorePromise(listArchivedSessions(dbPath))
