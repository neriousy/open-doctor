...
import { filteredCommandItems } from "./actions.js"
...
export function useCommandPalette(
  ...
  useEffect(() => {
    if (paletteSelected >= visibleCommandItems.length) {
      const next = Math.max(0, visibleCommandItems.length - 1)
      paletteSelectedRef.current = next
      setPaletteSelected(next)
    }
  }, [paletteSelected, visibleCommandItems.length])
  ...
  function moveCommandPalette(direction: 1 | -1) {
    ...
    const next = Math.max(0, Math.min(visibleCommandItems.length - 1, paletteSelectedRef.current + direction))
    paletteSelectedRef.current = next
    setPaletteSelected(next)
  }

  function runSelectedCommandPaletteItem() {
    const command = visibleCommandItems[paletteSelectedRef.current]
    if (!command) return
    setPaletteOpen(false)
    paletteQueryRef.current = ""
    setPaletteQuery("")
